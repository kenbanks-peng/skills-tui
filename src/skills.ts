...
import { readdirSync, existsSync, mkdirSync, rmSync, symlinkSync, cpSync, lstatSync } from "fs";
import { cacheDir } from "./config";
import type { AgentConfig } from "./config";
import { logCmd, ensureOpencode, isFileRepo, fileUrlToPath } from "./utils";
...
export async function loadInstalledSkills(repo: string, isGlobal: boolean): Promise<Set<string>> {
  try {
    ...
    for (const lockPath of lockPaths) {
      ...
      let pruned = false;
      for (const [skillName, info] of Object.entries(data.skills) as [string, any][]) {
...
export async function parseInstalledSkills(isGlobal: boolean, agentNames: string[]): Promise<InstalledSkillInfo[]> {
  try {
    const args = ["skills", "list"];
    if (isGlobal) args.push("-g");
    args.push("--agent", ...ensureOpencode(agentNames));
...
export async function loadSkillsFromRepo(repo: string): Promise<string[]> {
  try {
    ...
    // Fetch fresh data
    const args = ["skills", "add", repo, "--list"];
...
export function getInstalledLocalSkills(repoUrl: string, isGlobal: boolean): Set<string> {
  const baseDir = agentsSkillsDir(isGlobal);
...
export function installLocalSkill(
  ...
  for (const agent of agents) {
    ...
    if (agentPath === ".agents/skills/" || agentPath === "~/.agents/skills/") continue;
    const agentSkillsDir = expandHome(isGlobal ? agentPath : join(process.cwd(), agentPath));
...
export function removeLocalSkill(
  ...
  for (const agent of agents) {
    ...
    const agentPath = isGlobal ? agent.global : agent.local;
    if (agentPath === ".agents/skills/" || agentPath === "~/.agents/skills/") continue;
    const agentSkillDir = join(expandHome(isGlobal ? agentPath : join(process.cwd(), agentPath)), skillName);
    try {
      if (lstatSync(agentSkillDir).isSymbolicLink() || existsSync(agentSkillDir)) {
