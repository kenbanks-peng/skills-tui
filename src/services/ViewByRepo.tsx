...
import { loadInstalledSkills, loadSkillsFromRepo, listLocalSkills, getInstalledLocalSkills, installLocalSkill, removeLocalSkill } from "../skills";
import { truncateText, logCmd, ensureOpencode, isFileRepo, repoDisplayName } from "../utils";
...
export function ViewByRepo({
  ...
  const [allRepoSkills, setAllRepoSkills] = useState<Map<string, string[]>>(new Map());
  ...
  useEffect(() => {
    ...
    const loadAll = async () => {
      ...
      await Promise.all(repos.map(async (repo) => {
        const skills = isFileRepo(repo) ? listLocalSkills(repo) : await loadSkillsFromRepo(repo);
        map.set(repo, skills);
      }));
  ...
  useEffect(() => {
    if (filteredRepos.length > 0 && (!selectedRepo || !filteredRepos.includes(selectedRepo))) {
  ...
  const buildArgs = (action: "add" | "remove", repoUrl: string, skill?: string): string[] => {
    const args = ["skills", action];
    if (isGlobal) args.push("-g");
    if (skill) args.push("--skill", skill);
    args.push("--agent", ...ensureOpencode(selectedAgents));
    args.push("-y", repoUrl);
    return args;
  };
  ...
  const repoOptions = filteredRepos.map(repo => ({ name: repoDisplayName(repo) }));
  ...
  useKeyboard((key) => {
    ...
    if (key.name === "down") {
      const newIndex = Math.min(filteredSkills.length - 1, selectedSkillIndex + 1);
    ...
    if (key.name === "pagedown") {
      const newIndex = Math.min(filteredSkills.length - 1, selectedSkillIndex + adjustedViewportHeight);
      const maxOffset = Math.max(0, filteredSkills.length - adjustedViewportHeight);
    ...
    if (key.name === "end") {
      ...
      setSkillScrollOffset(Math.max(0, filteredSkills.length - adjustedViewportHeight));
      return;
    }
    if (key.name === "space") {
        ...
        if (isFileRepo(selectedRepo)) {
          try {
            if (isSelected) {
              ...
              installLocalSkill(selectedRepo, skill, isGlobal, agents, selectedAgents);
            }
            onInstallComplete();
          } catch (err) {
            onError(`ERROR while ${isSelected ? "removing" : "installing"} ${skill}`);
          }
        } else {
          if (isSelected) {
  ...
  const visibleSkills = filteredSkills.slice(skillScrollOffset, skillScrollOffset + adjustedViewportHeight);
  const hasMore = skillScrollOffset + adjustedViewportHeight < filteredSkills.length;
            ...
            <>
              {hasPrevious && (
                <box paddingLeft={1} marginBottom={1}>
                  <text fg={theme.overlay1}>{"\u2191"} {skillScrollOffset} more above</text>
                </box>
              )}
              <box flexDirection="column" gap={0} overflow="hidden" flexGrow={1}>
                {visibleSkills.map((skill, visibleIndex) => {
                  ...
                  return (
                    ...
                    >
                      <text
                        fg={isSelected ? theme.green : isHighlighted ? theme.yellow : theme.subtext1}
              ...
              {hasMore && (
                <box paddingLeft={1}>
                  <text fg={theme.overlay1}>
                    {"\u2193"} {filteredSkills.length - skillScrollOffset - adjustedViewportHeight} more below
                  </text>
                </box>
              )}
            </>
