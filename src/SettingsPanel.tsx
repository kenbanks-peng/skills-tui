...
export function SettingsPanel({
  ...
  const maxAgentSlots = Math.max(3, termHeight - 24);

  const hasLess = scrollOffset > 0;
  const hasMore = scrollOffset + maxAgentSlots < agents.length;
  const visibleAgentCount = maxAgentSlots - (hasLess ? 1 : 0) - (hasMore ? 1 : 0);
  const visibleAgents = agents.slice(scrollOffset, scrollOffset + visibleAgentCount);
  ...
  useKeyboard((key) => {
    ...
    if (key.name === "down") {
      ...
      const newAgentIndex = newIndex - 1;
      if (newAgentIndex >= 0 && newAgentIndex >= scrollOffset + visibleAgentCount) {
  ...
  return (
    ...
    >
      <box
        ...
        backgroundColor={activeSettingIndex === 0 ? theme.surface1 : "transparent"}
        paddingLeft={1}
        paddingRight={1}
      >
      ...
      <box height={1} />
      {hasLess && (
        <text fg={theme.overlay1} paddingLeft={2}>▲ {scrollOffset} more</text>
      )}
      {visibleAgents.map((agent, vi) => {
        ...
        return (
          <box
            ...
            paddingRight={1}
          >
            <text fg={isActive ? theme.yellow : isChecked ? theme.green : theme.subtext1}>
              {isChecked ? "[✓] " : "[ ] "}{agent.display}
            </text>
          </box>
        );
      })}
      {hasMore && (
        <text fg={theme.overlay1} paddingLeft={2}>▼ {agents.length - scrollOffset - visibleAgentCount} more</text>
      )}
    </box>
  );
}
